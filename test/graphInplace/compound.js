  ...
  describe('Basic graph functions', () => {
    describe('Compound Node functions', () => {
      it('gets nodes by compound path', () => {
        ...
        var graph = Graph.addNode(impl, toIP(Graph.empty()))
      ...
      it('does not confuse parent compounds and inner nodes', () => {
        var impl = Graph.addNode(
          ...
          toIP(Graph.compound({name: 'a', ports: [{port: 'out', kind: 'output', type: 'string'}]})))
        var graph = Graph.addNode(impl, toIP(Graph.empty()))
      ...
      it('checks nodes by compound path', () => {
        var impl = Graph.addNode(
          {name: 'a', ports: [{port: 'in', kind: 'input', type: 'number'}], atomic: true},
          toIP(Graph.compound({name: 'b', ports: [{port: 'out', kind: 'output', type: 'string'}]})))
        var graph = Graph.addNode(impl, toIP(Graph.empty()))
      ...
      describe('.addNodeIn', () => {
        ...
        it('callback returns root graph', () => {
          ...
          const newGraph = Graph.addNodeIn('b', {name: 'c', ports: [{port: 'in', kind: 'input', type: 'number'}], atomic: true}, toIP(graph),
            (n, g) => {
      ...
      it('removes a node on the root level', () => {
        ...
        var remGraph = Graph.removeNode('a', toIP(graph))
      ...
      it('removes nodes in compounds', () => {
        var impl = Graph.addNode({name: 'a', ports: [{port: 'in', kind: 'input', type: 'number'}], atomic: true},
          toIP(Graph.compound({name: 'b', ports: [{port: 'out', kind: 'output', type: 'string'}]})))
        var graph = Graph.addNode(impl, toIP(Graph.empty()))
        var remGraph = Graph.removeNode(['b', 'a'], toIP(graph))
      ...
      it('removes nodes in compounds in compounds', () => {
        var impl = Graph.addNode({name: 'a', ports: [{port: 'in', kind: 'input', type: 'number'}], atomic: true},
          toIP(Graph.compound({name: 'b', ports: [{port: 'out', kind: 'output', type: 'string'}]})))
        var impl2 = Graph.addNode(impl, toIP(Graph.compound({name: 'c', ports: [{port: 'out', kind: 'output'}]})))
        var graph = Graph.addNode(impl2, toIP(Graph.empty()))
        var remGraph = Graph.removeNode(['c', 'b', 'a'], toIP(graph))
      ...
      it('removes compounds that contain nodes', () => {
        ...
        var impl2 = Graph.addNode(impl, toIP(Graph.compound({name: 'c', ports: [{port: 'out', kind: 'output'}]})))
        var graph = Graph.addNode(impl2, toIP(Graph.empty()))
        var remGraph = Graph.removeNode(['c', 'b'], toIP(graph))
        expect(Graph.hasNode('»c»b»a', remGraph)).to.be.false
        expect(Graph.hasNode('c', remGraph)).to.be.true
      ...
      it('only removes specified node in compound', () => {
        ...
        var graph = Graph.addNode(impl, toIP(Graph.empty()))
        var remGraph = Graph.removeNode(['b', 'b'], toIP(graph))
      ...
      it('can address nodes relative to compound', () => {
        var impl = Graph.flow(
          Graph.addNode({name: 'a', ports: [{port: 'in', kind: 'input', type: 'number'}], atomic: true}),
          Graph.addNode({name: 'b', ports: [{port: 'in', kind: 'input', type: 'number'}], atomic: true})
        )(toIP(Graph.compound({name: 'c', ports: [{port: 'out', kind: 'output', type: 'string'}]})))
        var graph = Graph.addNode(impl, toIP(Graph.empty()))
      ...
      it('replaces references with a compound node', () => {
        var impl = Graph.flow(
          Graph.addNode({name: 'a', ports: [{port: 'in', kind: 'input', type: 'number'}], atomic: true}),
          Graph.addNode({name: 'b', ports: [{port: 'in', kind: 'input', type: 'number'}], atomic: true})
        )(toIP(Graph.compound({componentId: 'b', ports: [{port: 'out', kind: 'output', type: 'string'}]})))
        var graph = Graph.flow(
          Graph.addNode({ref: 'abc', name: '123'}),
          Graph.replaceNode('123', impl),
          {inPlace: true}
        )()
        expect(Graph.hasNode('123', graph)).to.be.true
        expect(Graph.nodes(graph)).to.have.length(1)
      ...
      it('replaces ports in short notation in edges when replacing', () => {
        ...
        )(toIP(Graph.compound({componentId: 'b', ports: [{port: 'out', kind: 'output', type: 'string'}, {port: 'in', kind: 'input', type: 'string'}]})))
        var graph = Graph.flow(
          Graph.addNode({ref: 'abc', name: '123'}),
          Graph.replaceNode('123', impl),
          {inPlace: true}
      ...
      it('Removing a node removes its edges', () => {
        ...
        expect(Graph.edgesDeep(Graph.removeNode('a', toIP(graph))).length).to.equal(0)
        expect(Graph.edgesDeep(Graph.removeNode('b', toIP(graph))).length).to.equal(0)
      })

      it('Adding a compound relabels all edges to the new id', () => {
        var comp = Graph.addEdge({from: '@inC', to: '@outC'},
          toIP(Graph.compound({name: 'c', ports: [{port: 'inC', kind: 'input'}, {port: 'outC', kind: 'output'}]})))
        ...
        var graph = Graph.addNode(comp, toIP(Graph.empty()))
